...
- **ETAPA 5 — CERTIFICAÇÃO TRANSVERSAL (Issue #256) — artefato novo `docs/etapa-5-certificacao.md`:** auditoria **somente documental** (nenhum código/migration/RPC/role/capability/teste novo), com **reutilização** das certificações e gates já registrados. Obrigações levantadas do roadmap e dos documentos do repositório (`docs/auditorias/2026-09-10-checkpoint-etapa-5-pos-f5-06.md` §3/§4, `docs/auditorias/auditoria-f5-09-preparacao-p5-p9.md` §4, §20 do `docs/F5-07-desenho-tecnico.md` e critérios §17.1 de cada fase); a **Issue #256 é ilegível** neste ambiente (sem `gh`). Resultado: os **três BLOCKERS históricos estão demonstradamente resolvidos** (ciclos → F5-09; colaboradores/histórico → F5-07/F5-08; metas/observações → F5-10/F5-11) e as **obrigações transversais** (trust boundaries, D1–D22, RLS/policies, ACL/grants, auditoria append-only, idempotência/concorrência, multi-tenant/IDOR, fail-closed, guardas de UI, CI completo, coerência promessa × prova) têm evidência citável. **NÃO há lacuna material bloqueante agora.** Dívidas **não bloqueantes** registradas (R1–R6): leitores legados de ciclo ainda presentes em módulos de produção (`cicloEquipeService`, `historicoOrganizacionalStorage`, `permissaoAvaliacao`, `cancelamentoCicloService`, `reaberturaCicloService`, `relatorioService` e várias páginas importando `cicloAvaliacaoStorage`) — a **verificar/decidir na F5-12** que nenhuma **decisão de autorização** os usa; `localCycleRepository` LEGADO declarado; fixtures que pré-carregam chaves legadas; prova literal de transferência entre organizações; defeito latente de diagnóstico no `41-validar-f5-11-p3.sql`; cenário `42` derivando tenant da P2. **Fora de escopo:** as 2 falhas pré-existentes de Windows/CRLF, a importação do acervo legado e o hardening/redesign de F6. O fechamento formal da Etapa 5 é a **F5-12** (validação integrada), que reutiliza esta matriz.
