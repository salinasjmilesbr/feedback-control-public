...
### Status final da Issue #87

**Concluída.** Validação integrada da estrutura organizacional implementada,
executada e documentada; PR aberto com `Closes #87` (sem merge).

### Resumo do que foi implementado

Um único par de artefatos de validação (sem migration, sem alteração de schema,
sem código funcional): cenário sintético determinístico/idempotente e runner de
asserts, exercitando F3-01..F3-09 em conjunto sobre uma organização 100%
sintética representativa do piloto.

### Arquivos criados/alterados

- `supabase/validacao/01-cenario-f3-10.sql` (criado) — organização sintética
  completa (prefixo `fc`), timeline histórica, colegiado v1→v2, autor
  `user_profiles`;
- `supabase/validacao/02-validar-f3-10.sql` (criado) — runner com 29 asserts;
- `supabase/validacao/README.md` (alterado) — seção F3-10;
- `supabase/README.md` (alterado) — título + seção "Validação executada (F3-10)";
- `docs/F3-10-desenho-validacao.md` (este documento) — desenho → decisões →
  execução → evidências → encerramento.

### Matriz final de cobertura dos 14 cenários

| # | Cenário | Resultado |
| --- | --- | --- |
| 1 | Gerente com três Coordenadores e colaboradores abaixo | ✅ PASS |
| 2 | Consultor respondendo diretamente ao Gerente | ✅ PASS |
| 3 | Analistas Júnior/Pleno/Sênior sob o mesmo Coordenador (mesma hierarquia) | ✅ PASS |
| 4 | Estagiário sob Coordenador e sob Gerente (sem seniority) | ✅ PASS |
| 5 | Gerência menor sem Coordenador | ✅ PASS |
| 6 | Especialista no mesmo patamar do Gerente, sem equipe direta | ✅ PASS |
| 7 | Posição vaga | ✅ PASS |
| 8 | Troca definitiva de ocupante (+ sucessão F3-09) | ✅ PASS |
| 9 | Transferência entre coordenações | ✅ PASS |
| 10 | Licença mantendo occupation | ✅ PASS |
| 11 | Substituição temporária | ✅ PASS |
| 12 | Pessoa ocupando duas posições simultâneas | ✅ PASS |
| 13 | Diretor reportando a Diretor (nível repetido) | ✅ PASS |
| 14 | Colegiado ausente / com membros / histórico por ciclo | ✅ PASS |

### Verificações e falhas

**29 verificações [PASS], 0 falhas** — idênticas nas duas execuções (runner
`02-validar-f3-10.sql` com `ON_ERROR_STOP`, `exit 0`).

### Rebuilds limpos

Dois `db reset` limpos (16 migrations em ordem + seed), cada um seguido de
cenário + runner; ambos sem intervenção manual e com `exit 0`.

### Suíte local

- `npm test` — 556 testes em 50 arquivos, aprovado;
- `npm run build` — aprovado (tsc + vite);
- `npm run lint` — aprovado;
- `git diff --check` — aprovado.

### RLS deny-by-default

Comprovado como `authenticated`: leituras retornam 0 linhas em `collaborators`,
`occupations` e `cycle_evaluation_responsibilities`; INSERT em `occupations`
negado. Nenhuma policy foi criada ou alterada (3 policies de identidade
preservadas).

### Ausência de dados reais, secrets e acesso remoto

Varredura sem secrets/credenciais (a senha sintética `virtus-senha-f3-10-local`
é marcada como local de teste, mesmo padrão da F2-10). Nomes, matrículas
(`FC-1xxx`/`FC-2001`) e e-mails (`@example.invalid`) são 100% sintéticos. Nenhuma
conexão a projeto remoto; validação apenas no Supabase local.

### Defeitos encontrados durante a validação

Nenhum defeito em F3-01..F3-09 (nada de migration/schema foi alterado). Apenas
dois ajustes internos ao próprio runner F3-10, corrigidos antes do primeiro
rebuild: tipo do array de profundidade (cenário 3) e reforço do assert do
Especialista (mesmo superior do Gerente).

### Riscos / limitações remanescentes

- Validação de fechamento da Fase 3; sem importação da planilha real, sem
  produção, sem autorização completa, sem relatórios finais;
- CI/CodeQL não aguardados (repositório público; sem merge realizado).

### PR e commit

- **PR:** https://github.com/salinasjmilesbr/feedback-control-public/pull/142
  (`Closes #87`);
- **Commit:** `eeaca2f` `test(org): validar estrutura sintetica representativa do piloto (F3-10)`
  (branch `chore/issue-87-f3-10-validacao-integrada`).

### Critérios de aceite da Issue #87 — atendidos?

**Sim, todos atendidos**: todos os 14 cenários representáveis sem campos
especiais por cargo; estrutura histórica reconstruível por data (5 datas); nenhum
cenário exige gestor fictício; Estagiário coberto explicitamente; dados 100%
sintéticos; matriz documentada; rebuild/testes/build/lint executados; sem merge.

### Fase 3 validada?

**Sim.** O modelo F3-01..F3-09 foi exercitado em conjunto com uma estrutura
representativa do piloto, reconstrução histórica por data, sucessão de avaliador
e RLS deny-by-default, com evidência reproduzível — a Fase 3 pode ser considerada
validada.
