...
## 22. Implementação e finalização (relatório)

Relatório da implementação da F4-02 (Issue #89), branch
`feat/issue-89-f4-02-authorization-scopes`, sobre o desenho aprovado
(D1–D18 fechadas + 14 invariantes).

### 22.1 Migrations criadas

- `supabase/migrations/20260908010000_authorization_scopes_membership_collaborator.sql`
  — modelo de escopos + vínculo membership→collaborator + funções de resolução.

### 22.2 Tabelas, funções e constraints criadas

- **Tabelas (3):** `membership_collaborator_links`,
  `access_role_assignment_scopes`, `access_role_assignment_unit_targets`.
- **Referência aditiva:** `uq_membership_access_role_assignments_id_organization`
  (em assignments, para a FK composta de tenant dos scopes).
- **Constraints (19):** PKs, uniques (`uq_membership_collaborator_links_membership`,
  `uq_access_role_assignment_scopes_assignment_type`,
  `uq_access_role_assignment_unit_targets_scope_unit` + uniques de referência),
  checks (`ck_*_status`, `ck_access_role_assignment_scopes_type` com os 6 tipos)
  e FKs `ON DELETE RESTRICT` compostas de tenant (membership, colaborador,
  assignment, unidade, autor).
- **Trigger:** `trg_access_role_assignment_unit_targets_scope_type`
  (`enforce_unit_target_scope_type` — target de unidade só em scope
  ORGANIZATIONAL_UNIT; sem polimorfismo genérico).
- **Funções (`SECURITY INVOKER`, `STABLE`):** `resolver_collaborador_vinculado`,
  `resolver_capabilities_escopos_efetivas` (capability × scope × unidade) e
  `resolver_alvos_escopo` (alvos por scope na data — SELF/DR/DESCENDANTS/UNIT/
  ORGANIZATION; ASSIGNED fail-closed na F4-02).
- **RLS:** habilitado nas três tabelas, **deny-by-default** (zero policies,
  zero grants a `authenticated`/`anon`).

### 22.3 Scopes efetivamente implementados

- Tipos armazenáveis: `SELF`, `DIRECT_REPORTS`, `DESCENDANTS`,
  `ORGANIZATIONAL_UNIT`, `ORGANIZATION`, `ASSIGNED`.
- `ORGANIZATIONAL_UNIT` com **target tipado por `unit_id`** (somente a unidade
  explícita, sem subunidades — D5); `ORGANIZATION` como scope **explícito**
  (ADMIN bootstrap/atribuição cria a linha — D14); `ASSIGNED` **sem** targets
  próprios nesta fase (derivação de F3-08/09 adiada para F4-03/F4-05 — D6).
- Scope pertence à **assignment** (nunca ao access_role — D3); múltiplos
  scopes por assignment (D4); revogação por status (D5/D11), sem exclusão
  física; assignment sem scope = **fail-closed** (D15).
- Substituição temporária (F3-06) **não persistida**: efeito resolvido por
  data pelos resolvers F3-07; mapa capability×responsibility_type é F4-05
  (D13).

### 22.4 Decisões D1–D18 implementadas

Todas implementadas: D1 (vínculo em tabela própria, 1 por membership), D2
(tabela filha de scopes), D3 (scope por atribuição), D4 (mesma role com N
scopes), D5 (UNIT só unidade, target `unit_id`), D6 (híbrida: derivar F3-08/09
— não duplicado — e targets tipados mínimos), D7 (sem vigência; revogação por
status), D8 (união de múltiplas positions via F3-07), D9 (responsável efetivo
no contexto vivo; histórico F3-08/09 soberano — resolvers data-driven), D10
(união sem precedência), D11 (revogação pai→filhos + granular), D12 (tenant por
FKs compostas), D13 (preparação; sem mapa nem persistência), D14 (ORGANIZATION
explícito), D15 (fail-closed), D16 (resolução na data), D17 (estruturais
exigem vínculo; ADMIN usa ORGANIZATION), D18 (INVOKER; sem DEFINER novo).

### 22.5 Testes executados e resultados

- **Validação de banco (Supabase local):** `01-cenario-f4-02.sql` +
  `02-validar-f4-02.sql` — **28 verificações [PASS], 0 falhas**, em **dois**
  `db reset` limpos (resultado idêntico; reprodutibilidade comprovada). Cobre
  os 20 pontos exigidos: mesma capability × scopes diferentes, SELF só
  vinculado, sem vínculo fail-closed, DIRECT_REPORTS por reporting line (não
  job_role), DESCENDANTS pela árvore F3, união de múltiplas positions, UNIT só
  a unidade (sem subunidades), ORGANIZATION limitado ao tenant, ADMIN+
  ORGANIZATION sem confidencial, assignment sem scope vazio, ASSIGNED não vira
  hierarquia, cross-tenant bloqueado, membership/profile disabled vazios,
  revogação do pai invalida filhos, revogação granular preserva histórico,
  posição vaga sem alvo artificial, contexto histórico não reescrito, RLS
  deny-by-default e rebuild reproduzível.
...
### 22.6 Riscos e limitações remanescentes

- `resolver_alvos_escopo`/`resolver_capabilities_escopos_efetivas` são
  `SECURITY INVOKER`: como `authenticated` retornam vazio (RLS deny) — o
  consumo real fica para a F4-03 quando houver policies;
- `ASSIGNED` é **tipo armazenável** mas não resolve alvos na F4-02 (derivação
  de colegiado/responsabilidade avaliativa é F4-03/F4-05) — fail-closed;
- o efeito de substituição temporária não é materializado (resolvido por data
  via F3-07), e o mapa capability×responsibility_type é F4-05 (não definido);
- cenário insere identidades `auth.users` diretamente via SQL (apenas banco
  local/sintético);
- allowlists provisórias e `verify_jwt=false` permanecem até substituto seguro.

### 22.7 Deixado para etapas posteriores

- **F4-03:** policy engine (composição capability+scope+alvo+data+estado) e
  consumo dos resolvers; **F4-05:** regra completa de substituição temporária
  (mapa capability×responsibility_type) e ASSIGNED de responsabilidades
  avaliativas; **F4-06:** acesso excepcional auditado (vigência/motivo);
  **F4-08:** policies finais por tabela e exposição a `authenticated`;
  migração das allowlists e UI de administração.
