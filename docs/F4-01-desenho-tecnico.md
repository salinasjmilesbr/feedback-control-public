...
## 14. Implementação e finalização (relatório)

Relatório da implementação da F4-01 (Issue #88), branch
`feat/issue-88-f4-01-authorization-capabilities-roles`, sobre o desenho
aprovado (decisões D1–D18 fechadas).

### 14.1 Migrations criadas

- `supabase/migrations/20260908000000_authorization_capabilities_access_roles.sql`
  — modelo de autorização (tabelas, constraints, triggers e funções
  server-side);
- `supabase/migrations/20260908000001_authorization_system_catalog.sql`
  — catálogo de sistema determinístico/versionado (capabilities + role `admin`).

### 14.2 Tabelas, funções e constraints criadas

- **Tabelas:** `capabilities`, `access_roles`, `access_role_capabilities`,
  `membership_access_role_assignments`.
- **Referência aditiva** em memberships:
  `uq_user_organization_memberships_id_organization`.
- **Constraints (23):** PKs, uniques (`uq_capabilities_code`,
  `uq_access_roles_organization_name`, `uq_access_role_capabilities_role_capability`,
  `uq_membership_access_role_assignments_membership_role`), checks
  (`ck_capabilities_code/name/status`, `ck_access_roles_name/status/system_scope`,
  `ck_membership_access_role_assignments_status`) e FKs `ON DELETE RESTRICT`
  (incluindo FK composta de tenant `(membership_id, organization_id)` e FK de
  autor `created_by → user_profiles`).
- **Unicidade parcial:** índice `uq_access_roles_system_name` (nome único das
  roles de sistema, `organization_id IS NULL`).
- **Triggers (5):** `set_updated_at` nas quatro tabelas +
  `trg_membership_access_role_assignments_role_organization`
  (`enforce_membership_role_within_organization` — role customizada de outra
  organização bloqueada).
- **Funções server-side (`SECURITY DEFINER`, `EXECUTE` só `service_role`):**
  `conceder_acesso_role`, `revogar_acesso_role` e
  `resolver_capabilities_efetivas` (predicado canônico de capabilities efetivas,
  sem escopo).
- **RLS:** habilitado nas quatro tabelas, **deny-by-default** (zero policies,
  zero grants a `authenticated`/`anon`).

### 14.3 Capabilities e roles de sistema criadas

- **21 capabilities globais** (notação `domínio.verbo`): administração de
  acesso (`membership.read/.manage`, `access_role.manage`), administração
  organizacional (`collaborator.read/.manage`, `org.structure.manage`,
  `org.catalog.manage`), configurações (`settings.manage`), ciclos
  (`cycle.read/.manage`), avaliações (`evaluation.read/.create/.write/.cancel/
  .reopen`), metas (`goal.read/.write/.approve`), observações
  (`observation.read/.write`) e relatórios (`report.read`).
- **1 access_role de sistema:** `admin` (is_system, organization_id NULL,
  active), atribuível por membership/organização.
- **Bundle `admin` (9 capabilities):** `membership.read`, `membership.manage`,
  `access_role.manage`, `collaborator.read`, `collaborator.manage`,
  `org.structure.manage`, `org.catalog.manage`, `settings.manage`, `cycle.read`
  — **sem** nenhuma capability de conteúdo confidencial.
- **Nenhuma** capability confidencial genérica (`confidential.*`) e **nenhuma**
  role `manager`/`collaborator` baseada em cargo.

### 14.4 Decisões D1–D18 implementadas

Todas as 18 decisões fechadas foram implementadas: D1 (capabilities globais),
D2 (roles de sistema + customizadas por org), D3 (somente via roles), D4
(múltiplas roles por membership — união), D5 (status active/disabled), D6
(sistema imutável/customizadas inativáveis, sem exclusão física), D7 (sem
vigência temporal; revogação por estado), D8 (unicidade de code/name), D9
(catálogo de sistema por migration determinística), D10 (tenant por FK composta
+ trigger), D11 (role→capability sobre catálogo global), D12 (tabela-âncora
enxuta, sem escopo), D13 (`created_by` autor mínimo), D14 (somente role
`admin`), D15 (code em `domínio.verbo`), D16 (mecanismo server-side mínimo,
sem ampliar RLS, sem remover allowlists), D17 (admin por membership/org), D18
(sem capability confidencial genérica; separação por domínio futura).

### 14.5 Testes executados e resultados

- **Validação de banco (Supabase local):** `01-cenario-f4-01.sql` +
  `02-validar-f4-01.sql` — **39 verificações [PASS], 0 falhas**, em **dois**
  `db reset` limpos (resultado idêntico; reprodutibilidade comprovada). Cobre
  os 12 pontos exigidos: independência de cargo, role agrupa capability, ADMIN
  sem collaborator, job_role/occupation não concedem, união de múltiplas roles,
  cross-tenant bloqueado, membership/perfil desabilitados sem autorização
  efetiva, ADMIN sem confidencial, sem exclusão física, RLS deny-by-default e
  rebuild reproduzível.
- **Suíte frontend:** `npm test` — **556 testes em 50 arquivos**, aprovados.
- **Build/lint:** `npm run build` (tsc + vite) e `npm run lint` aprovados;
  `git diff --check` aprovado.
- Nenhuma conexão ao Supabase remoto; dados 100% sintéticos.

### 14.6 Riscos e limitações remanescentes

- `resolver_capabilities_efetivas` é `SECURITY DEFINER` e `EXECUTE` apenas
  `service_role`: o predicado efetivo existe (testado), mas ainda **não é
  exposto a `authenticated`** — permanece assim até as policies da etapa que
  autorizar leitura (RLS deny-by-default por contrato).
- A checagem cross-tenant do lado da role é por **trigger** (roles de sistema
  têm `organization_id NULL` e não participam de FK composta); a FK composta
  garante o lado da membership.
- O cenário de validação insere identidades `auth.users` diretamente via SQL
  (somente banco local/sintético); não é caminho de produção.
- Unicidade parcial de nome das roles de sistema usa índice único parcial
  (exceção documentada à convenção F1-02 de "constraint unique").
- Allowlists provisórias (`INVITE_ADMIN_USER_IDS`, `verify_jwt=false`) **não**
  foram removidas (invariante 6) — a substituição ocorrerá nas etapas que
  migrarem as Edge Functions.

### 14.7 Deixado para F4-02 e etapas posteriores

- **Escopos** SELF, DIRECT_REPORTS, DESCENDANTS, ORGANIZATIONAL_UNIT,
  ORGANIZATION, ASSIGNED e escopo temporário derivado de substituição (F3-06) —
  F4-02;
- **Policies por tabela e leitura/escrita ampla** para `authenticated` (etapa
  posterior da F4 que autorizar exposição; a F4-01 permanece deny-by-default);
- **UI de administração** de roles/capabilities;
- **Capabilities de confidencialidade separáveis por domínio/recurso** (quando
  houver necessidade concreta; nenhuma genérica nesta fase);
- **Migração das allowlists/Edge Functions** para autorização por capability;
- **Acesso excepcional e auditoria avançada** (etapas posteriores da F4);
- Dados reais (fora de escopo em toda a Fase 4).
