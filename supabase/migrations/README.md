...
| `20260921000000_f5_09_p7_catalog_admin_bundle.sql` | F5-09 P7: reconciliação **ADITIVA e IDEMPOTENTE** do catálogo (**D28** / Q-F5-09-3 alternativa A) — `cycle.manage` entra no bundle administrativo de SISTEMA (`admin`, `c0000000-0000-4000-8000-0000000000f1`), tornando a gestão soberana de ciclo **executável em produção** (mitiga R9); `cycle.cancel`, `cycle.reopen` e `cycle.period.correct` permanecem **FORA dos bundles/roles DE SISTEMA** (`is_system = true`) e seguem **concedíveis por configuração explícita em roles CUSTOMIZADAS** — as guardas (preflight e final) restringem a proibição ao tipo de role e **não** bloqueiam concessões legítimas. **Nenhuma** capability nova (catálogo intacto), **nenhuma** remoção física (F5-04 D14), **nenhuma** alteração de atributo de capability e **nenhuma** alteração de schema/RLS/RPC/tabela/coluna. Reexecução é **segura**: o tamanho esperado do bundle é calculado a partir de `v_ja_existia` (+1 só na primeira execução) e prova-se que a relação `admin × cycle.manage` existe **exatamente uma vez**. Validadores: `11-validar-f5-09-p7.sql` (estado), `12-cenario/13-validar-f5-09-p7-d28.sql` (guardas por tipo de role A–H, idempotência e reaplicação da migration no CI). |
| `20260923000000_f5_10_p2_goals_rpc.sql` | F5-10 P2 (Issue #212): **OPERAÇÕES SOBERANAS de metas** — 7 RPCs `SECURITY INVOKER` com `search_path = public`, `EXECUTE` só `service_role`, preflight e guarda final fail-closed: **`meta_criar`** (identidade UUID gerada pelo banco, `version = 0`, ciclo `ATIVO` da linha soberana, colaborador do mesmo tenant, quota do banco e unicidade parcial revalidadas **sob o lock**; evento `CRIADA`), **`meta_editar`** (definição `descricao`/`kpi`/`valor_alvo`; `tipo` não é editável nesta fase — declaração explícita; evento `EDITADA` com before/after), **`meta_atualizar_progresso`** (`progresso_percentual` inteiro 0..100 — nunca derivado de texto —, `resultado_atual` e `data_ultimo_acompanhamento` do **servidor**; evento `PROGRESSO_ATUALIZADO`), **`meta_finalizar`** (1ª finalização explícita; `atingida` decide `ATINGIDA`/`NAO_ATINGIDA`; **não** exige, cria ou invalida aprovação; evento `FINALIZADA`), **`meta_revisar_finalizacao`** (operação explícita de revisão com `before_value` preservando **integralmente** o fechamento anterior — nunca sobrescrita silenciosa, sem estado `REABERTA`; evento `REVISAO_FINALIZACAO`) e **`meta_excluir`** (exclusão **apenas lógica**, motivo obrigatório, `excluida` + `data_exclusao`; evento `EXCLUIDA`), **`meta_definir_limites_do_ciclo`** (D21 — decisão de review: altera o limite de **um** tipo por **upsert** em `evaluation_cycle_goal_limits`, nunca abaixo das metas **não excluídas** daquele tipo, com `expected_version` **do ciclo**, `version` do ciclo **+1**, `operation_id` e evento `LIMITES_DO_CICLO_ALTERADOS` gravado em **`cycle_events`** — nunca em `evaluation_goal_events`). Todas: tenant revalidado por `(id, organization_id)` (cross-tenant = `NOT_FOUND`), ator/membership revalidados por `evaluation_ator_valido` (autoria **resolvida no banco** — membership nunca vem do corpo), `expected_version` obrigatório comparado **após** o lock e o `SELECT FOR UPDATE` (`F5_10_CONFLICT`), `version + 1` por mutação efetiva, idempotência por `unique (organization_id, operation_id)` da trilha com `payload_hash` SHA-256 **derivado server-side**, evento append-only na **mesma transação** (falha em qualquer parte ⇒ rollback total), serialização pela **mesma** família normativa de advisory lock dos ciclos (chave `evaluation_cycles:<organization_id>` via `ciclo_lock_organizacao` — **nenhuma** família/chave nova) e **zero `DELETE`/`TRUNCATE`**. **Anti-escopo respeitado:** nenhuma aprovação (P3), nenhuma matriz `goal.read/write/approve`, Policy Engine, RLS own-tenant ou RPC de leitura (P4), nenhum Edge/cliente (P5), nenhum cutover/backfill/frontend (P6), nenhuma bateria integrada (P7) e F5-11 intocada. **Decisão de review (D21):** `LIMITES_DO_CICLO_ALTERADOS` é registrado em **`cycle_events`** (limite é configuração do ciclo; capability normativa `cycle.manage`; `expected_version` e lock são do ciclo; a trilha de ciclo já possui `cycle_id`, FK tenant-bound, autoria soberana, `operation_id`, `payload_hash`, before/after e idempotência). O CHECK de `cycle_events.event_type` foi ampliado de forma **aditiva e fail-closed** (baseline conferido antes do `ALTER`) — **sem** anular `evaluation_goal_events.goal_id`, **sem** `goal_id` arbitrário, **sem** tabela nova de eventos, **sem** família de lock nova e **sem** editar migration histórica já mergeada da P1. O gate funcional `cycle.manage` (Policy Engine/Edge) permanece para P4/P5, registrado no comentário da RPC. Validadores: `21-cenario/22-validar-f5-10-p2.sql` (blocos M/D21); as listas fechadas de anti-antecipação da P1 (`20-validar-f5-10-p1.sql`, bloco K) e da F5-09/P9 (`15-validar-f5-09-p9.sql`) foram ampliadas para as 7 RPCs desta fase. |
