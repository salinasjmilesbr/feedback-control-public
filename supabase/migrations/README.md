...
| `20260922000000_f5_10_p1_goals_schema.sql` | F5-10 P1 (Issue #210): **SCHEMA, INTEGRIDADE e LIMITES** das metas soberanas — cria `evaluation_goals` (identidade **UUID** do banco; `cycle_id`/`collaborator_id` soberanos por **FK COMPOSTA** com `organization_id`, tornando cross-tenant impossível estruturalmente; `tipo`, conteúdo, `status` funcional, `progresso_percentual` **integer 0..100**, campos de fechamento, **soft delete** com coerência de campos, `version`), `evaluation_goal_approvals` (**FATO** de aprovação com autoria soberana por membership, motivo e `revogado_em` — nunca campo/status da meta; **único parcial** de aprovação vigente por papel), `evaluation_goal_events` (**trilha APPEND-ONLY** com autoria, `operation_id` único por organização, `payload_hash` SHA-256, CHECKs fechados de `entity_type`/`event_type` e **3 triggers** que negam UPDATE/DELETE/TRUNCATE inclusive ao owner e a `service_role`) e `evaluation_cycle_goal_limits` (**autoridade soberana da quota**, único `(cycle_id, tipo)`, 0..3). Invariantes de quota **no banco** (D20/D21): trigger recusa meta acima do limite (ausência de quota = **zero**, fail-closed), recusa reduzir a quota abaixo das metas vivas e recusa `DELETE` da linha de quota. RLS **deny-by-default integral** nas quatro tabelas (nenhuma policy, nenhum privilégio a `anon`/`authenticated`; `service_role` sem `DELETE`/`TRUNCATE`), preflight e guarda final fail-closed. **Anti-escopo respeitado:** nenhuma RPC funcional, nenhuma lógica de aprovação, nenhuma capability nova, nenhuma policy de leitura (P4), nenhum Edge/cliente/cutover. Validadores: `19-cenario/20-validar-f5-10-p1.sql` (com testes negativos) — a guarda de "não antecipar F5-10" do validador da F5-09/P9 passou a **lista fechada** dos objetos desta P1. |
